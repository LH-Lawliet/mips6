...
int adc_init(ADC_t *adc, uint32_t mode, OnSample cb)
{
	...
	if (adc == _ADC1) {
		...
		/* A COMPLETER */
		adc->CR1 = mode;
		//already done in sample  adc->CCR |= (1<<22); // enable vbat conversion
...
uint16_t adc_channel_sample(ADC_t *adc, uint32_t channel)
{
	...
	/* A COMPLETER */
	_ADC1->SQR[0] &= ~((n_to_convert&0xF)<<20);

	_ADC1->SQR[2] &= ~(0x1F);
	_ADC1->SQR[2] |= channel;


	_ADC1->CR2 |= ADC_CR2_SWSTART;

	
	while (_ADC1->SR & ADC_SR_EOC) {};

	uint16_t returnVal = _ADC1->DR;
	_ADC1->SR &= ~ADC_SR_EOC;
